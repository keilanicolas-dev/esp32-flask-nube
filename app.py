...
<head>
  ...
  <style>
    :root{
      ...
      --card:#e1e5ea;        /* gris más oscuro pero elegante */
      --cyan:#0b6cff;        /* color principal (azul) */
      --txt:#1a1a1a;         /* texto oscuro */
      --muted:#5f6b76;       /* texto secundario */
      --btn:#e6e9ed;         /* botones */
      --btnActive:#0b6cff;   /* botón activo */
    }
    ...
    *{box-sizing:border-box}
    ...
    .btn.active{background:var(--btnActive); color:#ffffff}
    .btn.csv{
      background:linear-gradient(135deg, rgba(11,108,255,.95), rgba(11,108,255,.75));
      color:#ffffff;
    }
    .cards{
      margin-top:10px;
    ...
    .card{
      background: radial-gradient(900px 320px at 50% 0px, rgba(255,255,255,.55), transparent 65%), var(--card);
      border:1px solid rgba(0,0,0,.06);
      border-radius:22px;
...
<script>
  ...
  // “cortes” para que después de limpiar solo entren nuevos
  let sinceIdS2 = null;
  let sinceIdW  = null;
  ...
  // ✅ y.min/y.max fijos: Corrientes 0-1, Radiómetro 0-105
    ...
    return new Chart(ctx, {
      ...
      options: {
        ...
        scales: {
          ...
          y: { min: 0, max: yMax, grid: { color: "rgba(0,0,0,.10)" } }
        }
      }
    });
  }
  ...
  const chCorr = makeLineChart("chCorr", "Corriente (A)", 1);     // ✅ 0-1
  const chPot  = makeLineChart("chPot",  "Potencia (W)", 200);
  const chRad  = makeLineChart("chRad",  "Radiómetro (%)", 105);   // ✅ 0-105

  // ===== WROOM =====
  const chWv1 = makeLineChart("chWv1", "Voltaje1", 30);
  const chWc1 = makeLineChart("chWc1", "Corriente1 (A)", 1);       // ✅ 0-1
  const chWp1 = makeLineChart("chWp1", "Potencia1", 200);

  const chWv2 = makeLineChart("chWv2", "Voltaje2", 30);
  const chWc2 = makeLineChart("chWc2", "Corriente2 (A)", 1);       // ✅ 0-1
  const chWp2 = makeLineChart("chWp2", "Potencia2", 200);

  const chWv3 = makeLineChart("chWv3", "Voltaje3", 30);
  const chWc3 = makeLineChart("chWc3", "Corriente3 (A)", 1);       // ✅ 0-1
  const chWp3 = makeLineChart("chWp3", "Potencia3", 200);
  ...
  document.getElementById("btnClear").addEventListener("click", async () => {
    try{
      ...
      console.log("Limpiadas. Desde ahora solo datos nuevos. sinceIdS2=", sinceIdS2, "sinceIdW=", sinceIdW);
    }catch(e){
